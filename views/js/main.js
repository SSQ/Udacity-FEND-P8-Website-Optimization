...
var resizePizzas = function(size) {
  ...
  changeSliderLabel(size);
   ...
   //  change pizza size and batch style change their widths
...
function updatePositions() {
  ...
  //compute scrollTop outside the original for loop to avoid repeating layout-style
  var docscroll = document.body.scrollTop / 1250;
  var phaseall = [];
  // result of i % 5 always consists of 5 numbers, so use i as index
  for (var i = 0; i < 5; i++) {
    var mathdocscro = Math.sin(docscroll + i);
    phaseall.push(mathdocscro);
  }
  //extract the computation out of for loop
  for (var i = 0, itemslength = items.length, phase; i < itemslength; i++) {
    phase = phaseall[i % 5];
    //console.log('phase:',phase,'document.body.scrollTop / 1250:',document.body.scrollTop / 1250);
